...
let write_build_options common_ctx filename defines =
   ...
   let cmd = try Process_helper.find_program "haxelib" with Not_found -> "haxelib" in
   let pin,pout,perr,pid = Process_helper.open_process_args_full_pid cmd [|"haxelib"; "path"; "hxcpp"|] (Unix.environment()) in
   writer#write ("hxcpp=" ^ (Pervasives.input_line pin));
   Pervasives.ignore (Process_helper.close_process_full_pid (pin,pout,perr,pid));
   writer#close;;
